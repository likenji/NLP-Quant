...
import jieba
jieba.load_userdict("C:\\Users\\User\\Desktop\\华南BOSS\\NLP+策略\\store\\test1.txt")
import math
...
def get_urls_info(url):
    ...
    temp_array=np.array([[int(bsobj.findAll('cite')[0].get_text()),
            ...
            ] for bsobj in bsObj.find("ul",{"class":"newlist"}).findAll("li")])
    return temp_array
    
names=["浏览数","回复数","发帖时间","回帖时间","贴吧内链","贴吧名字","帖子内链","帖子标题","作者","作者内链"]
pool=Pool()    
total_list=pool.map(get_urls_info,raw_pool)
temp=pd.DataFrame(np.concatenate(np.array(total_list),axis=0),columns=names)
